...
export class TableExampleComponent implements OnInit {
    ...
    sortColumn1() {
        if (this.column1SortDir === 'none' || this.column1SortDir === 'desc') {
        ...
        } else if (this.column1SortDir === 'asc') {
    ...
    sortDate() {
        if (this.dateSortDir === 'none' || this.dateSortDir === 'desc') {
            this.dateSortDir = 'asc';
            this.tableRows.sort((val1, val2) => {
                return +new Date(val1.date) - +new Date(val2.date);
            });
        } else if (this.dateSortDir === 'asc') {
